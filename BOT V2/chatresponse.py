...
def LUCABlockGetResponse(query, bot_purpose, FAISS_dir, users_mem_dir, user_id, webhookEventId, openai_key, newcomers_condition = "" ,load_message_count = 4 ,temperature = 0, bot_prefix = "AI", similarity_search_k = 2):
    ...
    if webhookEventId not in pastwebhookEventId:
        ...
        if memories and len(memories) > 0 and len(memories[-1]) > 0:
            ...
            # memory.save_context({"input": "[Started messaging]"}, {"output": "[Sees the message]"})
        ...
        Create only 1 message as {bot_prefix} for the following conversation then send the message.
        ...
        # query = query
        docs = vectorstore.similarity_search_by_vector(messageVector, k=similarity_search_k)
        ...
        with get_openai_callback() as cb:
            response = chain({"input_documents": docs, "special_condition": special_condition ,"time": timestamp_to_datetime(time_now),  "bot_purpose" : bot_purpose,  "input": user_message_with_salience, "bot_prefix" : bot_prefix}, return_only_outputs=True)['output_text']
        ...
        info = {'time': time(), 'vector': messageVector, 'message_user': query, 'message_ai': response, 'webhookEventId': webhookEventId, 'uuid': str(uuid4())}
