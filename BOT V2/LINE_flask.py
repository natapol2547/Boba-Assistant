...
def create_route(route_name, FAISS_dir, bot_purpose, bot_startwith, user_mem_dir, line_bot_api_key, channel_secret_key , openai_key, app, Thai_self = "ดิฉัน", temperature = 0, newcomers_condition = "", load_message_count = 3, similarity_search_k=2, sales_bot=False):
    ...
    def handle_message(event):
        ...
        if (bot_startwith in message.lower()) or source_type == "user" :
            ...
            response = LUCABlockGetResponse(query = message, bot_purpose= bot_purpose, FAISS_dir= FAISS_dir, users_mem_dir= user_mem_dir, user_id= user_id, webhookEventId= webhookEventId, openai_key= openai_key, temperature= temperature, newcomers_condition = newcomers_condition, bot_prefix=route_name, load_message_count=load_message_count,similarity_search_k=similarity_search_k)
            # print(response)
